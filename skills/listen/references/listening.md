...
every stdout line re-invokes the session. Arm it on `vitrinka work watch` and END
...
The human started this session through vitrinka's host: `vitrinka work listen`
...
`vitrinka work watch` loop, and injects each work item into the live session as a
...
grammar `vitrinka work watch` prints —
...
headless form: `vitrinka work watch --exec <harness>` resumes the listening thread
...
`vitrinka work listen` host started later for the same scope takes over cleanly —
